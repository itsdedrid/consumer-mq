      ...
      if (msg !== null) {
        ...
        try {
          ...
          if(data.queueId == null){
            const createdQueue = await createQueue(data.studentId, data.reqId, data.timeslotId, data.period, data.uid)
            console.log('Inserted into database:', createdQueue);
          }
          else{            
            const changedQueue = await changeQueue(data.queueId,data.studentId, data.reqId, data.timeslotId, data.period, data.uid)
